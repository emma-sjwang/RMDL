...
roi_size = 1067      # 32*70+551 = 2791   GetModel3    32*40+551 = 1831
inner_size = roi_size
inner_stride = 64
dense_coef = 1

dense_stride = inner_stride/dense_coef
ext_size = dense_stride * (dense_coef-1)
roi_ext_size = roi_size + ext_size
input_size = roi_ext_size
...
def BuildModel(include_top=False,
                      weights='imagenet',
                      input_tensor=None,
                      input_shape=(roi_size, roi_size,3),
                      pooling=None,
                      classes=1000, isextract=False):
    ...
    if include_top:
        ...
        x = Conv2D(1024, (1, 1), padding='valid', name="conv7")(x)
        extracted_feature = x
        if isextract:
            model = Model(inputs, extracted_feature, name='vgg16_extract')
            # model.summary()
            return model
        x = Activation('relu')(x)
        # x = Conv2D(1, (1, 1), activation="sigmoid", padding='valid', name="cls")(x)
        x = Conv2D(classes, (1, 1), activation="softmax", padding='valid', name="cls")(x)
